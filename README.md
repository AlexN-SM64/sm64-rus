...
```
sudo apt install git build-essential python3 binutils-mips-linux-gnu pkgconf libcapstone-dev
```
...
2. Клонируйте основной репозиторий, где есть исходный код игры, в каталог `sm64-rus`:
```
git clone https://github.com/sm64-port/sm64-port sm64-rus
```

3. Клонируйте дополнительный репозиторий, где есть русификация, в каталог `sm64-rus-base`:
```
git clone https://github.com/AlexN-SM64/sm64-rus sm64-rus-base
```

4. В проводнике находите каталог `sm64-rus-base`, скопируйте всё, кроме папки `.git`, и вставляйте в тот каталог репозитория, где есть исходный код игры, заменив файлы.
...
wsl --install
```
Важно: WSL требуется Windows 10 версия 1903 или новее для систем x64 (или версия 2004 или новее для систем ARM64). После установки WSL вам потребуется перезагрузить компьютер и войти снова, чтобы продолжить.

2. Перейдите в раздел "Linux" сверху и продолжайте следовать инструкцию по выполнению сборке.
...
1. Запустите терминал и установите следующие пакеты (Ubuntu или Debian):
```
sudo apt install git build-essential python3 pkgconf libusb-1.0-0-dev libsdl2-dev
```
Важно: Перед установкой сначала выполните обновление: `sudo apt-get update && sudo apt-get upgrade`
...
```
pacman -S git make python3 mingw-w64-ucrt-x86_64-gcc
```
...
4. Клонируйте основной репозиторий, где есть исходный код игры, в каталог `sm64-rus`:
```
git clone https://github.com/sm64-port/sm64-port sm64-rus
```

5. Клонируйте дополнительный репозиторий, где есть русификация, в каталог `sm64-rus-base`:
```
git clone https://github.com/AlexN-SM64/sm64-rus sm64-rus-base
```

6. В проводнике находите каталог `sm64-rus-base`, скопируйте всё, кроме папки `.git`, и вставляйте в тот каталог репозитория, где есть исходный код игры, заменив файлы.
...
- Выполнение сборки игры для Windows с поддержкой OpenGL практически НЕВОЗМОЖНО.
