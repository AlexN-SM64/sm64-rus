...
#ifndef VERSION_EU
static unsigned char textSoundModes[][TEXT_SOUND_MODES_LENGTH] = { { TEXT_STEREO }, { TEXT_MONO }, { TEXT_HEADSET } }; //! ONLY IN RUSSIAN VERSION
#endif
...
void exit_score_file_to_score_menu(struct Object *scoreFileButton, s8 scoreButtonID) {
    ...
    if (scoreFileButton->oMenuButtonState == MENU_BUTTON_STATE_FULLSCREEN
        ...
        play_sound(SOUND_MENU_CAMERA_ZOOM_OUT, DIFF3_G_GLOBAL_SOUND_SOURCE);
#if ENABLE_RUMBLE
        ...
        for (buttonID = MENU_BUTTON_SCORE_MIN; buttonID < MENU_BUTTON_SCORE_MAX; buttonID++) {
            ...
            if (check_clicked_button(buttonX, buttonY, 22.0f) == TRUE && sMainMenuTimer >= SCORE_TIMER) {
                ...
                if (buttonID == MENU_BUTTON_SCORE_RETURN || buttonID == MENU_BUTTON_SCORE_COPY_FILE
                    ...
                    play_sound(SOUND_MENU_CLICK_FILE_SELECT, DIFF3_G_GLOBAL_SOUND_SOURCE);
#if ENABLE_RUMBLE
                    queue_rumble_data(5, 80);
#endif
                    ...
                    if (sMainMenuTimer >= SCORE_TIMER) {
                        ...
                        if (save_file_exists(buttonID - MENU_BUTTON_SCORE_MIN) == TRUE) {
                            play_sound(SOUND_MENU_CAMERA_ZOOM_IN, DIFF3_G_GLOBAL_SOUND_SOURCE);
#if ENABLE_RUMBLE
                            queue_rumble_data(5, 80);
#endif
                            ...
                            play_sound(SOUND_MENU_CAMERA_BUZZ, DIFF3_G_GLOBAL_SOUND_SOURCE);
#if ENABLE_RUMBLE
                            queue_rumble_data(5, 80);
#endif
    ...
    if (sMainMenuTimer >= 5) {
        ...
        switch (sSelectedButtonID) {
            case MENU_BUTTON_PLAY_FILE_A:
                play_sound(SAVE_FILE_SOUND, DIFF3_G_GLOBAL_SOUND_SOURCE);
#if ENABLE_RUMBLE
...
void print_erase_menu_prompt(s16 x, s16 y) {
    ...
    if (sCursorClickingTimer == 2) {
        ...
        if (sEraseYesNoHoverState == MENU_ERASE_HOVER_YES) {
            play_sound(SOUND_MARIO_WAAAOOOW, DIFF3_G_GLOBAL_SOUND_SOURCE);
#if ENABLE_RUMBLE
            queue_rumble_data(5, 80);
#endif
...
#include "rus/file_select_erase_fileletter.inc.c" //! INCLUDED ONLY IN RUSSIAN VERSION
#ifdef VERSION_EU
...
#include "rus/file_select_score_fileletter.inc.c" //! INCLUDED ONLY IN RUSSIAN VERSION
